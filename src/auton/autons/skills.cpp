...
void runSkills() {
  ...
  lemlib::Pose barrierCornerSweepTarget {
      TILE_LENGTH * 1.8,
      TILE_LENGTH * 2 - Robot::Dimensions::drivetrainWidth / 2 - 1};
  Robot::chassis->moveToPose(barrierCornerSweepTarget.x,
                             barrierCornerSweepTarget.y, LEFT, 2000,
                             {.forwards = false, .minSpeed = 64});
  Robot::Actions::expandBackWing();
  // wait until near pose
  ...
  tank(127, 127, 300, 3);
  // ram into goal
  Robot::chassis->moveToPoint(0, -10000000, 700,
                              {.forwards = false, .minSpeed = 127});
  Robot::chassis->waitUntilDone();
  // back out of goal
  tank(64, 127, 400, 0);
